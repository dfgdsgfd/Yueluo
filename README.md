...
默认会创建来自中国校园与城市生活场景的中文演示用户、分类、标签、图文/视频帖子、评论、点赞、收藏、关注、通知、钱包/积分、礼品卡、系统公告和 IM 会话。命令是幂等的，重复执行会补齐缺失数据，并把 demo 用户密码重置为本次指定值；已存在的英文旧演示数据也会刷新为中文版本，不会重复创建相同用户或帖子。
